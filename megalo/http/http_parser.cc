...
static megalo::ConfigVar<uint64_t>::ptr g_http_request_buffer_size =
    megalo::Config::Lookup("http.request.buffer_size"
                ,(uint64_t)(4 * 1024), "http request buffer size");

static megalo::ConfigVar<uint64_t>::ptr g_http_request_max_body_size =
    megalo::Config::Lookup("http.request.max_body_size"
                ,(uint64_t)(64 * 1024 * 1024), "http request max body size");

static megalo::ConfigVar<uint64_t>::ptr g_http_response_buffer_size =
    megalo::Config::Lookup("http.response.buffer_size"
                ,(uint64_t)(4 * 1024), "http response buffer size");

static megalo::ConfigVar<uint64_t>::ptr g_http_response_max_body_size =
    megalo::Config::Lookup("http.response.max_body_size"
...
void on_request_method(void* data, const char* at, size_t length){
  ...
  if(m == HttpMethod::INVALID_METHOD) {
    MEGALO_LOG_WARN(g_logger) << "invalid http request method: "
...
void on_http_version(void* data, const char* at, size_t length){
  ...
  if(strncmp(at, "HTTP/1.1", length) == 0){
    ...
    MEGALO_LOG_WARN(g_logger) << "invalid http request version: "
        << std::string(at, length);
...
void on_http_field(void *data, const char *field, size_t flen, const char *value, size_t vlen){
  HttpRequestParser* parser = static_cast<HttpRequestParser*>(data);
  if(flen == 0) {
    MEGALO_LOG_WARN(g_logger) << "invalid http request field length == 0";
...
HttpRequestParser::HttpRequestParser()
  :m_error(0){
  m_data.reset(new megalo::http::HttpRequest);
...
void on_response_http_field(void *data, const char *field, size_t flen, const char *value, size_t vlen){
  ...
  if(flen == 0) {
    MEGALO_LOG_WARN(g_logger) << "invalid http response field length == 0";
    parser->setError(1002);
    return;
  }
  parser->getData()->setHeader(std::string(field, flen)
...
void on_response_status(void* data, const char* at, size_t length){
  HttpResponseParser* parser = static_cast<HttpResponseParser*>(data);
  megalo::http::HttpStatus status = (megalo::http::HttpStatus)(atoi(at));
...
HttpResponseParser::HttpResponseParser()
  :m_error(0){
  m_data.reset(new megalo::http::HttpResponse);
